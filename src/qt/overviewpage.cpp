...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::PACT)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("PACT")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizePayPACTAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePayPACTAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizePayPACTAmount = strAnonymizePayPACTAmount.remove(strAnonymizePayPACTAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePayPACTAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizePayPACTAmount * COIN) nMaxToAnonymize = nAnonymizePayPACTAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizePayPACTAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizePayPACTAmount));
        strAnonymizePayPACTAmount = strAnonymizePayPACTAmount.remove(strAnonymizePayPACTAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePayPACTAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
